...
export function inboxRoutes(ctx: AdminCtx) {
  ...
  const approveSchema = z.object({
    ...
    autoTranslateLangs: z.array(z.string().length(2)).optional(),
  ...
  r.post('/:jid/approve', async (c) => {
    ...
    repo.upsertGroup({
      ...
      enabled: true,
      voiceTranslate: args.voiceTranslate ?? true,
      textTranslateOnMention: args.textTranslateOnMention ?? true,
      autoTranslateLangs: (args.autoTranslateLangs ?? []).filter((l) => args.targetLanguages.includes(l)),
